...
if test -n "$USE_ICU"; then
    ...
    if test -z "$MOZ_SYSTEM_ICU"; then
        dnl if test -z "$YASM" -a -z "$GNU_AS" -a "$COMPILE_ENVIRONMENT"; then
        dnl    AC_MSG_ERROR([Building ICU requires either yasm or a GNU assembler. If you do not have either of those available for this platform you must use --without-intl-api])
        dnl fi
